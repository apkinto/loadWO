...
def getRest( url, session, payload, query, requestHeader, authorization, recordLimit, log, count, proxies, *argv ):
	#log = setLogging()
	payload = ''
	...
	try:
		r = session.get( url, data=payload, headers=requestHeader, params=querystring, auth=authorization, proxies=proxies )
		print (r.url)
...
def postBatchRest( url, session, partsList, n, authorization, log, count, proxies ):
	#log = setLogging()
	start = getTime()
	batchHeader={'Cache-Control': 'no-cache','Content-Type': 'application/vnd.oracle.adf.batch+json', 'Connection': 'close', 'REST-Framework-Version': "4"}
	urlObject = parseUrl(url)
	...
	for c in chunksList:
		...
		partsBody['parts'] = c
		try:
			r = session.post( url, json=partsBody, headers=batchHeader, auth=authorization, proxies=proxies )
			time = getTime() - start
